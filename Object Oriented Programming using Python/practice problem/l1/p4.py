...
class GarmentOrder:
    ...
    def calculate_amount(self):
        return GarmentOrder.garment_dict[self.get_cloth_type()][1] * self.get_no_of_piece()
    def update_stock(self):
        self.__delivery_date = date.today() + timedelta(days = GarmentOrder.garment_dict[self.get_cloth_type()][2])
        GarmentOrder.garment_dict[self.get_cloth_type()][0] -= self.get_no_of_piece()
        
    def take_order(self):
        self.__cloth_type = self.get_cloth_type().lower()
        if self.get_cloth_type().lower() not in GarmentOrder.garment_dict.keys():
            return -1
        if GarmentOrder.garment_dict[self.get_cloth_type().lower()][0] < self.get_no_of_piece():
            return -1
        self.update_stock()
        return self.calculate_amount()
    
g = GarmentOrder('saree', 15)
GarmentOrder.garment_dict={'trousers': [40, 600, 30], 'shirt': [70, 800, 40], 'saree': [25, 1000, 20], 'jersey': [15, 500, 10]}
g.take_order()